/**
 * Ejecuta base-datos/actualizar-v6-correo.sql (correo del publicador)
 */
...
let sql = fs.readFileSync(path.join(__dirname, '..', 'base-datos', 'actualizar-v6-correo.sql'), 'utf8');
sql = sql.replace(/USE nzoaqaxydg_zonagrupos_db;/, `USE ${nombre};`);

const tmp = path.join(__dirname, '..', 'logs', '_tmp_correo.sql');
...
try {
  ...
  console.log('\n✓ Columna correo_publicador aplicada (v6).');
} catch {
