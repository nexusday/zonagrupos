/**
 * Ejecuta base-datos/actualizar-v3-reportes.sql (tabla reportes)
 */
...
let sql = fs.readFileSync(path.join(__dirname, '..', 'base-datos', 'actualizar-v3-reportes.sql'), 'utf8');
sql = sql.replace(/USE nzoaqaxydg_zonagrupos_db;/, `USE ${nombre};`);

const tmp = path.join(__dirname, '..', 'logs', '_tmp_reportes.sql');
