/**
 * Ejecuta base-datos/actualizar-v7-correos-contacto.sql
 */
...
let sql = fs.readFileSync(path.join(__dirname, '..', 'base-datos', 'actualizar-v7-correos-contacto.sql'), 'utf8');
sql = sql.replace(/USE nzoaqaxydg_zonagrupos_db;/, `USE ${nombre};`);

const tmp = path.join(__dirname, '..', 'logs', '_tmp_correos_contacto.sql');
...
try {
  ...
  console.log('\n✓ Tabla correos_contacto aplicada (v7).');
} catch {
