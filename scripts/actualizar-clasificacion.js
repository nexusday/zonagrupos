/**
 * Ejecuta base-datos/actualizar-v5-clasificacion.sql (UTF-8 emojis + clasificación +18)
 */
...
let sql = fs.readFileSync(path.join(__dirname, '..', 'base-datos', 'actualizar-v5-clasificacion.sql'), 'utf8');
sql = sql.replace(/USE nzoaqaxydg_zonagrupos_db;/, `USE ${nombre};`);

const tmp = path.join(__dirname, '..', 'logs', '_tmp_clasificacion.sql');
...
try {
  ...
  console.log('\n✓ UTF-8 emojis y clasificación +18 aplicados (v5).');
} catch {
